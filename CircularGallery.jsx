...
function createTextTexture(gl, text, font = 'bold 28px Arial', color = '#8a5a32') {
...
class Title {
  constructor({ gl, plane, text, textColor = '#8a5a32', font = 'bold 28px Arial' }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
    this.text = text;
  ...
  onResize() {
    ...
    const textHeight = this.plane.scale.y * 0.11;
    const textWidth = textHeight * aspect;

    this.mesh.scale.set(textWidth, textHeight, 1);
    this.mesh.position.y = -this.plane.scale.y * 0.62;
  }
}
...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          ...
          p.z += sin(p.x * 4.0 + uTime) * 0.10;
          p.z += cos(p.y * 3.0 + uTime * 0.8) * 0.08;
          p.y += sin(p.x * 2.2 + uTime * 0.6) * 0.03 * (1.0 + abs(uSpeed));
      ...
      fragment: `
        ...
        void main() {
          ...
          vec3 warm = vec3(1.0, 0.97, 0.93);
          color.rgb = mix(color.rgb, color.rgb * warm, 0.10);
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.x = (this.viewport.width * (620 * this.scale)) / this.screen.width;
...
class App3D {
  ...
  createMedias(items, bend = 1.15, textColor, borderRadius, font) {
    const defaultItems = [
      ...
      { image: '/images/coffee-7.jpg', text: 'Caramel Latte' },
...
export default function CircularGallery({
  ...
  useEffect(() => {
    const app = new App3D(containerRef.current, {
      items,
      bend,
      textColor,
      borderRadius,
